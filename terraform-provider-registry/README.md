...
terraform {
  required_providers {
    registry = { source = "registry.local/sasokolov/registry" }
  }
}

provider "registry" {}   # REGISTRY_ENDPOINT and REGISTRY_TOKEN
...
| Resource | Manages |
| --- | --- |
| `registry_feed` | A feed: upstream connector, hosting, publishers, redirect, policy chain, federation behaviour |
...
`registry_token` writes a working credential into Terraform state, because the
...
provider_installation {
  dev_overrides {
    "registry.local/sasokolov/registry" = "/path/to/bin"
  }
  direct {}
}
```
