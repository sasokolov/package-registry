...
пользователь явно попросит (обсудить с пользователем в конце фазы). — Спрошено
и пересмотрено: 2026-07-29 пользователь попросил свой модуль, он сделан в
Фазе 16.
...
- [x] Аутентификация в браузере: консоль ходит тем же
      `Authorization`-заголовком, что и npm/maven, — своей сессии у неё нет.
      Форма строится из `GET /api/v1/auth/methods`: сайт сам говорит, что
      принимает, и как это получить. Токен живёт в sessionStorage;
      localStorage — только по явному согласию. `whoami` решает, какие
      действия показывать.
- [x] Браузерный вход через сторонний OIDC (authorization code + PKCE):
      у issuer'а с `client_id` в форме кнопка «войти», а не поле для
      вставки id_token'а. Verifier/state/nonce — в sessionStorage вкладки,
      обмен кода делает реестр (не зависит от CORS у провайдера, работает
      с confidential-клиентом), `redirect_uri` и адрес issuer'а реестр
      вычисляет сам. Результат — обычный id_token: те же проверки, тот же
      аудит, отзыв у провайдера отзывает и консоль. Секрет клиента — через
      `client_secret_env`, не в документе (инварианты 8 и 12).
- [x] Conformance: SPA отдаётся, deep link не 404, ассеты кэшируются и
      ...
      unit-тестами в `ui/embed_test.go`. Браузерный вход прогоняется целиком
      против настоящего провайдера в контейнере (`98_browser_signin.sh`,
      `conformance/fake-oidc`): кнопка, редирект, согласие, обмен, работающий
      креденшл — и отказы, ради которых PKCE и существует (код без verifier'а,
      повторный код, чужой nonce, ненастроенный issuer).
...
- [x] Поиск в hosted-фидах npm (`/-/v1/search`) и Composer
      (`/search.json`) — из тех же фактов, из которых собираются packument
      и p2. Composer-фид объявляет search в корневом манифесте; у прокси
      он больше не удаляется, а перенаправляется на реестр, потому что
      теперь реестр умеет его обслужить.
- [x] npm-документ поиска отдаёт `keywords`, `maintainers`, `links` и
      `date` всегда, даже пустыми: клиент npm разворачивает их без
      проверки, и отсутствующее поле роняет `npm search`, а не сокращает
      вывод.
- [x] Поиск склеивается и в npm-, и в composer-группах — по той же
      причине, что и в NuGet: hosted-член отвечает всегда.
- [x] Conformance: `npm search` и `composer search` находят
      опубликованный пакет; пустой ответ на несуществующий термин; поиск
      группы помечен `X-Registry-Merged`.

---

## Фаза 14 — RBAC в стиле Vault

Именованные политики, capabilities, явный `deny`, привязка к данным из
аутентификации. Решения (в отсутствие ответа — рекомендованные варианты):
пространство имён из объектов реестра, а не из URL; старые поля
компилируются в те же политики; привязка отдельными bindings.

- [x] `core/access`: политика = имя + правила; правило = path + capabilities.
      Capabilities: `read`, `list`, `publish`, `create`, `update`, `delete`,
      `deny`. По умолчанию — отказ.
- [x] Пространство имён: `feed/<фид>/<координата>` для содержимого и
      `sys/<config|tokens|quarantine|conflicts|replication|status>` для
      администрирования. Не URL: ядро не знает раскладки форматов
      (инвариант 1), и правило одинаково работает для фида и для группы.
- [x] Матчинг как в Vault: точный путь бьёт glob; среди glob'ов выигрывает
      наиболее специфичный (меньше `+`, длиннее литеральный префикс); на
      выбранном уровне `deny` бьёт всё. Это позволяет «запретить широко,
      разрешить узко» — ровно то, ради чего deny и нужен.
- [x] `bindings`: сопоставление данных аутентификации (kind, issuer,
      subject/имя токена, project_path, ref) со списком политик. Политика
      не знает, кому она выдана.
- [x] Старые `anonymous` / `publishers` / `admins` компилируются в
      политики на загрузке: один движок, одна семантика, ничего не
      ломается.
- [x] Объяснимость: `GET /api/v1/access/explain` отвечает, какое правило и
      какая политика приняли решение. Отказ, который нельзя объяснить, —
      это отказ, который будут обходить.
- [x] Группы: право проверяется у каждого члена по его собственному пути,
...
- [x] Conformance: запрет узкого пути поверх широкого разрешения; grant
      узкого поверх широкого запрета; deny поверх всего на одном уровне;
      привязка по claim'ам GitLab; explain объясняет решение.

Ещё одно, найденное при реализации и закреплённое тестом: генерируемые
политики обязаны встречаться друг с другом на одном уровне специфичности.
Точное правило `sys/quarantine` из `sys:read` перебивало `sys/*`
администратора и молча лишало его права ставить карантин — это поведение
Vault, и оно кусается ровно там, где политики пересекаются.

**Acceptance:** те же сценарии доступа, что и сегодня, выражаются
политиками и проходят (28/28 conformance без правок в остальных
сценариях); плюс невозможные раньше — публикация только в свой namespace
координат, deny целого фида с узким исключением, карантин без права
трогать конфигурацию (`96_access_policies.sh`).

---

## Фаза 15 — Инвентарь и учёт использования

Предпосылка: пользователь запросил (2026-07-29) видеть статус и по хостингу,
и по прокси — сколько пакетов, сколько места, сколько скачиваний — и метрики
по фидам и группам, не по пакетам.

- [x] `core/usage`: периодический обход стораджа считает, что лежит в каждом
      фиде. Хостинг — одним запросом из `hosted_manifests` (его источник
      правды), прокси-кэш — обходом `manifests/<feed>/`: строк у него нет
      намеренно, чтобы скачивание переживало падение PG (инвариант 7).
      Обход пропускает манифесты, уже учтённые из БД, и идёт под
      межрепличным advisory-локом (инвариант 9).
- [x] Счётчики трафика: в памяти, флаш в PG пачкой по интервалу. Запрос
      никогда не ждёт счётчика; при падении реплики теряется дельта с
      последнего флаша — число на экране, а не факт, на который опираются.
      Неудачный флаш возвращает дельты обратно, а не выбрасывает: иначе
      недоступность БД выглядела бы как тишина.
- [x] Байты: у фида — с учётом общего блоба у каждого владельца («во сколько
      обходится фид»), у сайта — по одному разу («счёт за хранилище»).
      Разница вынесена в `shared_bytes` и отвечает на «сколько освободится,
      если удалить фид».
- [x] Координата в манифесте кэша, иначе «сколько пакетов» для прокси не
      имеет ответа. Старые манифесты считаются в артефактах и байтах, но не
      в пакетах: пробел закрывается по мере обновления кэша.
- [x] Группы: свой трафик (что спросили у её URL) + сумма по членам как
      инвентарь, с пометкой. Проходной ответ считается и на группу, и на
      ответившего члена; склеенный документ — только на группу
      (`source: merged`); итог по сайту = не-групповые фиды + `merged`.
...
- [x] Топ пакетов по скачиваниям: `package_downloads` (feed, coordinate),
      ...
      и на детали фида. Только артефакты (метаданные — это резолв, а не
      установка), у группы свой топ, таблица ограничена с обеих сторон:
      20k координат в памяти между флашами и тысяча на фид в БД.
- [x] Conformance (`99_usage.sh`): прокси отчитывается о том, что реально
      закэшировал; хостинг считается из БД; запрос через группу считается
      один раз; итог по сайту не больше суммы фидов; метрики есть и ни одна
      не размечена координатой; анониму не отдаётся ничего.

Найдено живой проверкой и закреплено тестом: уборка «фидов, которых больше
нет» должна получать весь список настроенных фидов, а не только те, у кого
есть инвентарь. У группы своего хранилища нет и строки в отчёте тоже, но
трафик есть — иначе каждый обход обнулял счётчики всех групп.

**Acceptance:** `/api/v1/usage` и страница Usage показывают хостинг и прокси
одинаково; `registry_store_bytes` совпадает с тем, что в хранилище;
кардинальность метрик ограничена числом фидов.

---

## Фаза 16 — Helm и OCI

Предпосылка: пользователь запросил (2026-07-29) поддержку Helm-чартов «как в
ChartMuseum» и docker-образов. Оба формата были в списке «вне плана до явного
запроса» — запрос есть.

### Helm

- [x] `modules/format/helm`: прокси (`index.yaml` мутабельный с TTL и SWR,
      `charts/*.tgz` иммутабельные), хостинг в форме ChartMuseum
      (`POST /api/charts`, multipart или тело целиком), группа с мержем
      индексов, листинг `GET /api/charts[/{name}[/{version}]]`.
- [x] Имя и версия берутся из `Chart.yaml` внутри архива, а не из имени
      файла: и то и другое может содержать дефис, и имя файла не говорит,
      где граница. При проксировании — эвристика (версия начинается с
      цифры), и авторитетом остаётся индекс апстрима.
- [x] URL чартов в проксированном индексе переписываются на этот фид; чарт,
      живущий на CDN, едет закодированным в пути (как dist у composer) —
      иначе его нельзя проксировать вовсе.
- [x] Удаление версии не поддерживается: инвариант 4. Отказ 403 говорит про
      карантин, а не «нет такого пути».
- [x] `modules/internal/semver`: общий компаратор для npm и helm вместо
      второй копии. Maven намеренно не пользователь — у него своя схема.
- [x] Conformance `44_helm.sh` настоящим клиентом: `helm search`, `helm
      pull` через прокси и через группу, `helm template` хостового чарта,
      повторная публикация → 409, удаление → 403, публикация без права →
      403.

Найдено при этом и исправлено в ядре: `Reindex` смотрел только на то, умеет
ли **формат** хостинг, и не смотрел, хостит ли **фид**. Сгенерированный
пустой индекс писался по тому же ключу, что и прокси-кэш, — то есть
переконфигурация прокси-фида опустошала его до истечения TTL. У helm это
видно сразу (индекс один на репозиторий), у composer было бы то же самое с
`packages.json`.

### OCI (docker-образы)

- [x] `modules/format/oci`: прокси Distribution v2 (манифест по тегу —
      мутабельный с TTL и SWR, манифест и блоб по digest — иммутабельные и
      верифицируемые бесплатно, digest прямо в URL), хостинг (сессионная
      загрузка блобов, публикация манифестов), группа с объединением
      `tags/list` и `_catalog`.
- [x] Имя фида живёт внутри имени образа: `хост/oci/<фид>/<репозиторий>`.
      Клиент строит путь сам и всегда обращается к `/v2/` в корне, поэтому
      точка монтирования не может стоять впереди — новая возможность
      `api.FeedRouter`, дальше запрос идёт по обычной цепочке.
- [x] Тег — мутабельная координата, манифест по digest — иммутабельная.
      Перевешивание `latest` это протокол по назначению; релиз, на который
      тег указывал, остаётся на месте (инвариант 4). Удаление — 403 с
      указанием на карантин.
- [x] Тег читается прямо из опубликованной координаты (а не из
      сгенерированной копии): один источник правды и pull переживает
      недоступность Postgres (инвариант 7).
- [x] Аутентификация докер-клиента: `WWW-Authenticate: Basic` вместо общего
      по умолчанию (`api.AuthChallenger`) — docker берёт первую схему, для
      которой у него есть обработчик, и на Bearer уходит искать
      несуществующий token service.
- [x] Ядро: handshake с апстримом, который отвечает 401 и указывает, где
      взять токен (Docker Hub, ghcr, quay — иначе прокси не получает
      ничего); `Accept` в интенте (без него апстрим отдаёт другой документ);
      media type документа хранится вместе с ним и переживает кэш.
- [x] Conformance `45_oci.sh`: настоящий `registry:2` апстримом и настоящий
      OCI-клиент (crane) — pull через прокси, push в хостинг, переезд тега,
      неизменность digest, манифест без блобов → 400, удаление → 403,
      группа, публикация без права → 403. `docker pull`/`docker push`
      проверены на dev-стенде, где есть демон.

**Acceptance:** `helm` и `docker` работают против реестра как против
ChartMuseum и Distribution соответственно; conformance гоняет оба настоящими
клиентами.

---

## Фаза 17 — стенд в деплой-форме и сквозной смоук

Предпосылка: пользователь попросил (2026-07-29) прогнать смоук по всем фидам
на dev-стенде и с репликацией, и чтобы в каждом инстансе было больше одной
реплики — то есть проверять и HA.

- [x] `make dev-ha`: тот же dev-стенд в форме, в которой это разворачивают, —
      две stateless-реплики за nginx на том же порту, общий объектный стор,
      общая БД и один конфигурационный документ из стораджа. Всё, что ломается
      от состояния в процессе, видно на ноутбуке: правка конфигурации,
      доехавшая до одной реплики; сессия загрузки, которую может продолжить
      только она; «межрепличный» лок, оказавшийся мьютексом. Балансировщик
      намеренно без stickiness и ставит `X-Replica` — «правда ли это
      балансируется» должно быть заголовком, а не догадкой.
- [x] Провайдер идентичности переехал на адрес docker-моста: это единственный
      адрес, который одинаково означает одно и то же для браузера на хосте и
      для реплики внутри сети. На 127.0.0.1 реплика приходила бы к себе.
- [x] `make smoke` (`conformance/smoke/run.sh`): живой смоук по всем семи
      форматам против уже поднятого стенда. Для каждого формата одно и то же:
      поставить настоящее через группу (то есть через прокси), опубликовать
      своё в hosted-фид, достать опубликованное обратно через группу настоящим
      клиентом. Не герметичен намеренно — `make conformance` доказывает
      протоколы против фикстур, а это доказывает развёрнутый экземпляр. Ничего
      не вендорится: проект для сборки клонируется, свои пакеты генерируются.
- [x] На сайте, который не владеет hosted-фидом, смоук ждёт журнал и печатает,
      сколько ждал: публикация уходит на home-сайт и возвращается фактом.
      Замерено на двухсайтовом стенде — 1–2 секунды до появления в индексах;
      точная координата читается сразу (peer fallback).
- [x] `conformance/smoke/replicated.sh` — доехало ли опубликованное на другой
      сайт и чем оно отдано; `conformance/smoke/failover.sh` — что видит
      клиент, когда реплику убирают из-под него.

Найдено смоуком и исправлено: **пересылка публикации на home-сайт теряла
заголовки, описывающие тело**. `dotnet nuget push` — это multipart, и
home-сайт, собиравший запрос заново из метода, пути и тела, не видел пакет
внутри конверта: он сохранял конверт. Загрузка принималась, версия появлялась
в списке, реплицировалась на оба сайта — и ломался только `dotnet restore`.
Теперь едут заголовки о теле (Content-Type, Content-Range, Content-Encoding,
Content-Disposition, Accept) и не едет ничего о том, кто публикует (инвариант
14). Пункт «Publish-форвардинг» Фазы 7 закрывал механику пересылки, но не то,
что вместе с телом надо везти его описание.

Измерено на HA-стенде: остановка и возврат реплики под нагрузкой — 100
запросов, 0 потерь (клиент внутри сети). С хоста в тот же момент теряется один
запрос — это docker переписывает NAT опубликованного порта, не реестр. Реплику
надо выводить из балансировщика до остановки (в k8s это делает readiness-проба);
выдернутая без вывода реплика стоит запросов, которые были в полёте на ней.
...
Поиск по пакетам сверх того, что просит протокол (протокольный — NuGet
`v3/query`, npm `/-/v1/search`, Composer `/search.json` — сделан в Фазе 13);
PyPI/Cargo-модули
(архитектура их допускает — добавляются как обычные FormatModule).
Helm и OCI перенесены в план (Фаза 16) по запросу пользователя 2026-07-29. Гео-репликация перенесена в план (Фаза 7) по запросу
