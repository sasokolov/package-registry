...
- [ ] `core/config`: источник конфигурации абстрагирован (`file` | `store`),
...
- [ ] `ui/`: React + TypeScript + Vite, сборка в `ui/dist`, встраивание
      через `go:embed`; `make build` собирает UI и бинарь, Dockerfile —
      multi-stage, CI — отдельная джоба.
- [ ] Отдача на `/` с SPA-фолбэком для deep links; ассеты с
      контент-хэшем и иммутабельным кэшем; `/api` и `/ui` зарезервированы
      от имён форматов и фидов.
- [ ] Экраны: обзор сайта, фиды, пакеты фида с поиском, детали версии,
      репликация и гео, конфликты, карантин, токены, редактор конфигурации.
- [ ] Аутентификация в браузере: статический токен либо OIDC (если
      сконфигурирован); токен не попадает в localStorage без явного
      согласия, `whoami` определяет доступные действия.
- [ ] Conformance: SPA отдаётся, deep link не 404, ассеты кэшируются,
      API-эндпоинты отвечают, действия оператора работают из браузерного
      флоу (headless).
