...
import (
	...
	"github.com/fondaco-dev/fondaco/core/server"
)

// configCmd implements `fondaco config check -config <path>`: parse and
...
func configCmd(args []string, out io.Writer) error {
	if len(args) == 0 || args[0] != "check" {
		return errors.New("usage: fondaco config check [-config <path>]")
	}
	flags := flag.NewFlagSet("fondaco config check", flag.ContinueOnError)
