...
// tokenCmd implements `fondaco token create|revoke -name <name>`.
...
func tokenCmd(args []string, out io.Writer) error {
	if len(args) == 0 || (args[0] != "create" && args[0] != "revoke") {
		return errors.New("usage: fondaco token create|revoke -name <name> [-config <path>]")
	}
	sub := args[0]
	flags := flag.NewFlagSet("fondaco token "+sub, flag.ContinueOnError)
