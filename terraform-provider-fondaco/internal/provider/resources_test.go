...
`,
			Check: resource.ComposeAggregateTestCheckFunc(
				resource.TestCheckResourceAttr("fondaco_token.ci", "id", "tf-acc-ci"),
				resource.TestCheckResourceAttrSet("fondaco_token.ci", "secret"),
				// Eight characters of the hash, never the secret.
				resource.TestMatchResourceAttr("fondaco_token.ci", "hash_prefix",
					regexp.MustCompile(`^[0-9a-f]{8}$`)),
				// And the secret it handed back actually authenticates.
				checkSecretWorks(t, "fondaco_token.ci"),
			),
		}},
	})
}
...
func TestAccQuarantine(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: `
resource "fondaco_quarantine" "cve" {
  feed       = "hosted"
...
`,
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("fondaco_quarantine.cve", "reason", "manual"),
					resource.TestCheckResourceAttr("fondaco_quarantine.cve", "id",
						"hosted/maven:com.example:tf-acc@1.0.0/manual"),
					resource.TestCheckResourceAttrSet("fondaco_quarantine.cve", "created_at"),
			...
			{
				ResourceName:      "fondaco_quarantine.cve",
				ImportState:       true,
...
func TestAccQuarantine_refusesToDeclareAConflictBlock(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{{
			Config: `
resource "fondaco_quarantine" "derived" {
  feed       = "hosted"
