...
data "fondaco_feeds" "all" {
  depends_on = [fondaco_feed.seen]
}

data "fondaco_feed" "seen" {
  name       = fondaco_feed.seen.name
  depends_on = [fondaco_feed.seen]
}
...
`,
			Check: resource.ComposeAggregateTestCheckFunc(
				...
				resource.TestCheckResourceAttr("data.fondaco_feed.seen", "format", "maven"),
				resource.TestCheckResourceAttr("data.fondaco_feed.seen", "anonymous", "true"),

				// The feed just created appears in the listing, alongside the
				// ones the site was configured with by hand.
				resource.TestCheckTypeSetElemAttr("data.fondaco_feeds.all", "names.*", "tf-acc-seen"),
				resource.TestCheckTypeSetElemAttr("data.fondaco_feeds.all", "names.*", "hosted"),
