...
data "fondaco_access_explain" "acme_ci_publish" {
...
data "fondaco_access_explain" "acme_ci_internal" {
...
check "access_is_what_we_meant" {
  assert {
    condition     = data.fondaco_access_explain.acme_ci_publish.allowed
    error_message = "acme CI can no longer publish its own artifacts: ${data.fondaco_access_explain.acme_ci_publish.reason}"
  }

  assert {
    condition     = !data.fondaco_access_explain.acme_ci_internal.allowed
    error_message = "acme CI gained publish on the internal namespace via ${data.fondaco_access_explain.acme_ci_internal.policy}"
  }
}

# Which rule decided, for the runbook entry that explains a refusal.
output "why" {
  value = "${data.fondaco_access_explain.acme_ci_publish.policy} at ${data.fondaco_access_explain.acme_ci_publish.rule}"
}
