...
- 2026-07-26: кросс-репличный advisory lock деградирует в lock-less ingest при недоступном Postgres (инвариант 7); безопасно из-за идемпотентных контент-адресуемых записей.- 2026-07-26: hosted-манифесты пишутся ТОЛЬКО через `CoreServices.Publish`; иммутабельность держит unique-констрейнт PG (не advisory lock), S3-манифест — проекция для read-path без БД.
